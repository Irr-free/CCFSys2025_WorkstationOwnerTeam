...
/*
	// write rowstarts and column into file
	char offset_filename[300];
	char *tmpfile = getenv("TMPFILE");
	printf("[WriteTMP] %s\n", tmpfile);
	strcpy(offset_filename, tmpfile);
	strcat(offset_filename, "_off.txt");
	FILE* offset_fp = fopen(offset_filename, "w");
	for (int i = 0; i < g->nlocalverts; i++) {
		fprintf(offset_fp, "%d\n", rowstarts[i]);
	}
	fclose(offset_fp);
	printf("[WriteTMP] Write Off Complete!\n");

	char dst_filename[300];
	strcpy(dst_filename, tmpfile);
	strcat(dst_filename, "_dst.txt");
	FILE* dst_fp = fopen(dst_filename, "w");
	for (int i = 0; i < g->nlocaledges; i++) {
		fprintf(dst_fp, "%d\n", COLUMN(i));
	}
	fclose(dst_fp);
	printf("[WriteTMP] Write Dst Complete!\n");
*/
}
