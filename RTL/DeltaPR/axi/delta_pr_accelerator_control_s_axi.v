...
#(parameter
    ...
    output wire [31:0]                   one_over_n,
    output wire [31:0]                   added_const,
    output wire [31:0]                   delta2_value,
...
//        others - reserved
// 0x10 : Data signal of one_over_n
//        bit 31~0 - one_over_n[31:0] (Read/Write)
// 0x14 : reserved
// 0x18 : Data signal of added_const
//        bit 31~0 - added_const[31:0] (Read/Write)
// 0x1c : reserved
// 0x20 : Data signal of delta2_value
//        bit 31~0 - delta2_value[31:0] (Read/Write)
// 0x24 : reserved
// 0x28 : Data signal of bitmap_compressed_num
...
localparam
    ...
    ADDR_IER                          = 7'h08,
    ADDR_ISR                          = 7'h0c,
    ADDR_ONE_OVER_N_DATA_0            = 7'h10,
    ADDR_ONE_OVER_N_CTRL              = 7'h14,
    ADDR_ADDED_CONST_DATA_0           = 7'h18,
    ADDR_ADDED_CONST_CTRL             = 7'h1c,
    ADDR_DELTA2_VALUE_DATA_0          = 7'h20,
    ADDR_DELTA2_VALUE_CTRL            = 7'h24,
    ADDR_BITMAP_COMPRESSED_NUM_DATA_0 = 7'h28,
    ADDR_BITMAP_COMPRESSED_NUM_CTRL   = 7'h2c,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_one_over_n = 'b0;
    reg  [31:0]                   int_added_const = 'b0;
    reg  [31:0]                   int_delta2_value = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_ONE_OVER_N_DATA_0: begin
                    rdata <= int_one_over_n[31:0];
                end
                ADDR_ADDED_CONST_DATA_0: begin
                    rdata <= int_added_const[31:0];
                end
                ADDR_DELTA2_VALUE_DATA_0: begin
                    rdata <= int_delta2_value[31:0];
                end
...
assign one_over_n            = int_one_over_n;
assign added_const           = int_added_const;
assign delta2_value          = int_delta2_value;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_one_over_n[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_ONE_OVER_N_DATA_0)
            int_one_over_n[31:0] <= (WDATA[31:0] & wmask) | (int_one_over_n[31:0] & ~wmask);
    end
end

// int_added_const[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_added_const[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_ADDED_CONST_DATA_0)
            int_added_const[31:0] <= (WDATA[31:0] & wmask) | (int_added_const[31:0] & ~wmask);
    end
end

// int_delta2_value[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_delta2_value[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_DELTA2_VALUE_DATA_0)
            int_delta2_value[31:0] <= (WDATA[31:0] & wmask) | (int_delta2_value[31:0] & ~wmask);
    end
end
