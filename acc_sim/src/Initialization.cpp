...
#if STORE_DATA
    // write file
    const string prefix = "./edge_channel_small_";
    for (int i = 0; i < PSEUDO_CHANNEL_NUM; i++) {
        FILE* edge_fp;
        ...
        for (int j = 0; j < V_Num/CORE_NUM; j++){
            for (int k = i * GROUP_CORE_NUM; k < (i+1) * GROUP_CORE_NUM;k++){
                fprintf(edge_fp, "%d\n", Offset_URAM[k][j][0]);
            }
            for (int k = i * GROUP_CORE_NUM; k < (i+1) * GROUP_CORE_NUM;k++){
                fprintf(edge_fp, "%d\n", Offset_URAM[k][j][1]);
        ...
        printf("channel_%d_size=%d\n", i, Edge_MEM[i].size() + V_Num/CORE_NUM * GROUP_CORE_NUM * 2);
        fclose(edge_fp);
    }
#endif
...
void Initialize_Edge_bram(int V_Num) {
...
#if STORE_DATA
    // write file
    const string prefix = "./first_edge_small_";
    for (int i = 0; i < CORE_NUM; i++) {
        ...
        edge_fp = fopen(cu_filename.c_str(), "w");
        char  * hex_str;
        for (int j = 0; j < VTX_NUM/CORE_NUM; j++) {
            hex_str = inttohex (First_Edge_BRAM[i][j]);
            fprintf(edge_fp, "%s\n", hex_str);
        }
        printf("first_edge_%d_size=%d\n", i, VTX_NUM/CORE_NUM);
        fclose(edge_fp);
    }
#endif
...
#if STORE_DATA
    // write file
    const string prefix = "./vis_bitmap_small_";
    for (int i = 0; i < CORE_NUM; i++) {
        FILE* edge_fp;
        string cu_filename = prefix;
        cu_filename += to_string(i);
...
//            for(int k=BitMap_Compressed_Length-1; k>=0;k--){
//                if(Init_Bitmap[i][j].v[k])
//                    fprintf(edge_fp, "1");
//                else
//                    fprintf(edge_fp, "0");
//            }
//            fprintf(edge_fp, "\n");
        ...
        printf("bitmap_%d_size=%d\n", i, BitMap_Compressed_NUM);
        fclose(edge_fp);
    }
#endif
}