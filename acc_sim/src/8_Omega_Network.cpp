...
extern int clk;
extern int rst_rd;
extern int P1_Global_Iteration_ID_wr;
extern vector<LRUCache<int, int>> caches;
...
void Omega_Network(int Om1_Front_Push_Flag[], int Om1_Front_Update_V_ID[], int Om1_Front_Update_V_Value[], int Om1_Front_Pull_First_Flag[], int Om1_Front_Update_V_DValid[], int Om1_Front_Iteration_End[], int Om1_Front_Iteration_End_DValid[],
    ...
    //static vector<LRUCache<int, int>> caches(CORE_NUM, LRUCache<int, int>(1024));
    static vector<vector<int>> cache_queue_value(CORE_NUM);
    static vector<vector<int>> cache_queue_id(CORE_NUM);
    static vector<vector<int>> cache_queue_pull_first_flag(CORE_NUM);
    static vector<vector<int>> cache_queue_push_flag(CORE_NUM);
    static int count_all=0,count_del=0,count_del_first=0;
    static int iteration_id = 0;
    if(iteration_id!=P1_Global_Iteration_ID_wr){
        iteration_id=P1_Global_Iteration_ID_wr;
        for (int i = 0; i < CORE_NUM; ++ i)caches[i].clear();
        for (int i = 0; i < CORE_NUM; ++ i)if(!cache_queue_id[i].empty())exit(11);
    }
    if(clk%500==0){
        cout<<"count_all:"<<count_all<<endl;
        cout<<"count_del:"<<count_del<<endl;
        cout<<double(count_del_first)/double(count_all)<<endl;
        cout<<double(count_del)/double(count_all)<<endl;
    }
    for (int i = 0; i < CORE_NUM; ++ i) {
        if(Om1_Front_Update_V_DValid[i] && !Om1_Front_Push_Flag[i] && caches[i].get(Om1_Front_Update_V_Value[i])!=0){//&&Om1_Front_Pull_First_Flag[i]
            tmp_om1_push_flag_in[0][i] = 0;
            tmp_om1_v_id_in[0][i] = 0;
            tmp_om1_v_value_in[0][i] = 0;
            tmp_om1_pull_first_flag_in[0][i] = 0;
            tmp_om1_v_valid_in[0][i] = 0;
            tmp_om1_iteration_end[0][i] = Om1_Front_Iteration_End[i];
            tmp_om1_iteration_end_dvalid[0][i] = Om1_Front_Iteration_End_DValid[i];
            tmp_om1_stage_full[OMEGA_DEPTH][i] = Source_Core_Full[i];
//            cache_queue_value[i].push_back(caches[i].get(Om1_Front_Update_V_Value[i]));
//            cache_queue_pull_first_flag[i].push_back(Om1_Front_Pull_First_Flag[i]);
//            cache_queue_id[i].push_back(Om1_Front_Update_V_ID[i]);
//            cache_queue_push_flag[i].push_back(Om1_Front_Push_Flag[i]);
            cache_queue_value[Om1_Front_Update_V_ID[i]%CORE_NUM].push_back(caches[i].get(Om1_Front_Update_V_Value[i]));
            cache_queue_pull_first_flag[Om1_Front_Update_V_ID[i]%CORE_NUM].push_back(Om1_Front_Pull_First_Flag[i]);
            cache_queue_id[Om1_Front_Update_V_ID[i]%CORE_NUM].push_back(Om1_Front_Update_V_ID[i]);
            cache_queue_push_flag[Om1_Front_Update_V_ID[i]%CORE_NUM].push_back(Om1_Front_Push_Flag[i]);
            if(Om1_Front_Pull_First_Flag[i])count_del_first++;
        }
        else{
            tmp_om1_push_flag_in[0][i] = Om1_Front_Push_Flag[i];
            tmp_om1_v_id_in[0][i] = Om1_Front_Update_V_ID[i];
    ...
    for (int i = 0; i < CORE_NUM; ++ i) {
        ...
        #endif
        if(tmp_om2_v_valid_in[OMEGA_DEPTH][i]){
            Om2_Push_Flag[i] = tmp_om2_push_flag_in[OMEGA_DEPTH][i];
            Om2_Update_V_ID[i] = tmp_om2_v_id_in[OMEGA_DEPTH][i];
            Om2_Update_V_Value[i] = tmp_om2_v_value_in[OMEGA_DEPTH][i];
            Om2_Pull_First_Flag[i] = tmp_om2_pull_first_flag_in[OMEGA_DEPTH][i];
            ...
            Om2_Iteration_End[i] = tmp_om2_iteration_end[OMEGA_DEPTH][i];
            Om2_Iteration_End_DValid[i] = tmp_om2_iteration_end_dvalid[OMEGA_DEPTH][i];
            if(!tmp_om2_push_flag_in[OMEGA_DEPTH][i])count_all++;
        }
        else if(!cache_queue_id[i].empty()){
            Om2_Push_Flag[i] = cache_queue_push_flag[i][0];
            Om2_Update_V_ID[i] = cache_queue_id[i][0];
            Om2_Update_V_Value[i] = cache_queue_value[i][0];
            Om2_Pull_First_Flag[i] = cache_queue_pull_first_flag[i][0];
            Om2_Update_V_DValid[i] = 1;
            Om2_Iteration_End[i] = 0;
            Om2_Iteration_End_DValid[i] = 0;
            cache_queue_value[i].erase(cache_queue_value[i].begin());
            cache_queue_id[i].erase(cache_queue_id[i].begin());
            cache_queue_push_flag[i].erase(cache_queue_push_flag[i].begin());
            cache_queue_pull_first_flag[i].erase(cache_queue_pull_first_flag[i].begin());
            count_all++;
            count_del++;
        }
        else{
